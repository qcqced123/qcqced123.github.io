...
```
마지막 층의 인코더 출력값에는 `layernorm`을 적용해줘야 함을 잊지 말자. 

한편`layer_output`는 레이어 별 어텐션 결과를 시각화 하거나 나중에 `WeightedLayerPool`에 사용하려고 만들었다. 모든 층의 출력값을 층 정규화 레이어에 통과시켜 리스트에 저장하고 있다.
#### `🤖 VisionTransformer`  
